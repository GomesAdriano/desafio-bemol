  ...
  return (
    ...
    onHide={() => {
      ...
      <Modal.Body>
        <Row>
          <Col md={3}>
            <img src={produtoSelecionado.imagem} alt={produtoSelecionado.nome} className='imagem-compra' />
          </Col>
          <Col md={9}>
            <p className='descricao-compra'>{produtoSelecionado.descricao}</p>
            <p className='preco-compra'>R$ {(Number(produtoSelecionado.preco)).toFixed(2).replace('.', ',')}</p>
          </Col>
        </Row>
        <hr />
        ...
        <Form.Select className="mt-3" value={parcela} onChange={(e) => setParcela(e.target.value)}>
          <option value="0" disabled>Parcelas</option>
          <option value="1">x1 de {(Number(produtoSelecionado.preco) / 1).toFixed(2).replace('.', ',')}</option>
          <option value="2">x2 de {(Number(produtoSelecionado.preco) / 2).toFixed(2).replace('.', ',')}</option>
          <option value="3">x3 de {(Number(produtoSelecionado.preco) / 3).toFixed(2).replace('.', ',')}</option>
          <option value="4">x4 de {(Number(produtoSelecionado.preco) / 4).toFixed(2).replace('.', ',')}</option>
        </Form.Select>
        <div className='flex mt-3'>
          <p><strong>Saldo Atual:</strong> R$ {(Number(saldo)).toFixed(2).replace('.', ',')}</p>
          <p>
      ...
      <Modal.Footer>
        ...
        <Button variant="primary" 
          ...
          disabled={parcela===0}>Finalizar compra</Button>
      </Modal.Footer>
    </Modal>
